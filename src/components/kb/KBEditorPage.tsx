...
import { ArrowLeft, Save, Eye, Clock, Bold, Italic, Heading1, Heading2, List, ListOrdered, Link2, Code, Quote, AtSign, Undo, Redo, MoreHorizontal } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Separator } from "@/components/ui/separator";
import { kbCategories } from "@/data/kbMockData";
...
export function KBEditorPage({
  ...
  const ToolbarButton = ({ icon: Icon, label, active }: { icon: any; label: string; active?: boolean }) => (
    <button
      className={`p-2 rounded-lg hover:bg-muted transition-colors ${active ? "bg-muted text-primary" : "text-muted-foreground hover:text-foreground"}`}
      title={label}
    >
